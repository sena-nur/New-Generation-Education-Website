...
export const Blog = () => {
  ...
  return (
    <>
      <div
        id="blogComponent"
        className="max-sm:mx-8 max-md:mx-16 max-lg:mx-48 max-2xl:mx-24 max-xl:mx-4 mx-48 mb-48"
      >
        ...
        <div className="max-lg:gap-48 max-xl:gap-4 max-lg:grid-cols-1 grid grid-cols-3 gap-8">
          {data.map((item) => (
            <div key={item.id} className=" relative">
              <div className="z-10 bottom-32 right-12 absolute">
                <button
                  type="button"
                  className="max-xl:hidden text-white bg-category-primary hover:bg-deep-orange transition-all text-2xl rounded-full p-7 text-center inline-flex items-center me-2"
                >
                  <svg
                    className="w-4 h-4"
                    aria-hidden="true"
